...
class _ProductListScreenState extends State<ProductListScreen> {
                ...
                : _products.isEmpty
                    ...
                    : LayoutBuilder(
                        builder: (context, constraints) {
                          ...
                          int count = 2;
                          if (w >= 1400) count = 5;
                          else if (w >= 1100) count = 4;
                          else if (w >= 700) count = 3;
  ...
  String _ini(String n){ final p=n.trim().split(RegExp(r'\s+')); if(p.length>=2) return (p[0][0]+p[1][0]).toUpperCase(); return p[0].substring(0,p[0].length>=2?2:1).toUpperCase();}
  Widget _colorBox(String name)=> Container(color: _cFor(name), child: Center(child: Padding(padding: const EdgeInsets.all(10), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Container(width: 36, height: 36, decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.22), borderRadius: BorderRadius.circular(10)), child: Center(child: Text(_ini(name), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 14)))), const SizedBox(height: 6), Text(name, textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 12))]))));
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              color: AppColors.surfaceSoft,
              child: _colorBox(p['name'] as String),
            ),
            Expanded(
              child: Padding(
