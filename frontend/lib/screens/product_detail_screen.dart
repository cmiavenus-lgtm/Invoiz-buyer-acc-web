...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
      ...
      return ListView(
        ...
        children: [
          Row(
            ...
            children: [
              Expanded(child: _gallerySection(context, p)),
                  ...
                  children: [
                    Container(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(_fmt(_unitPrice), style: TextStyle(color: AppColors.primary, fontSize: 24, fontWeight: FontWeight.bold)),
                          ...
                          Row(
                            children: [
                              ...
                              Text(p.sold > 0 ? '${p.sold} sold · ${p.stock} left in stock' : '${p.stock} left in stock', style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                            ],
                          ),
                        ],
                      ),
                    ...
                    if (p.variants.isNotEmpty)
                      Container(
                        ...
                        color: AppColors.card,
                        padding: const EdgeInsets.all(14),
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Variations', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold)), const SizedBox(height: 10), ..._buildVariantGroups(p.variants)]),
                      ),
                    Container(
                      margin: const EdgeInsets.only(top: 8),
                      color: AppColors.card,
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Row(children: [
                          ...
                          Container(width: 44, height: 36, alignment: Alignment.center, decoration: BoxDecoration(color: AppColors.background), child: Text('$_quantity', style: const TextStyle(fontSize: 16))),
          ...
          Container(margin: const EdgeInsets.only(top: 16), color: AppColors.card, padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Product Details', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold)), const SizedBox(height: 8), Text(p.description ?? 'No description.', style: const TextStyle(fontSize: 13, height: 1.5))])),
          _specificationsSection(context, p),
          Container(margin: const EdgeInsets.only(top: 8, bottom: 20), color: AppColors.card, padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Ratings & Reviews', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold)), const SizedBox(height: 8), _reviewsSection()])),
        ],
      );
    }
    // Mobile fallback
  ...
  Color _cFor(String n){ final h=n.codeUnits.fold<int>(0,(a,c)=>a+c); const pal=[Color(0xFF0F766E),Color(0xFF1D4ED8),Color(0xFF7C3AED),Color(0xFFDB2777),Color(0xFFEA580C),Color(0xFF16A34A),Color(0xFF0891B2),Color(0xFF475569)]; return pal[h%pal.length];}
  String _iniFor(String n){ final p=n.trim().split(RegExp(r'\s+')); if(p.length>=2) return (p[0][0]+p[1][0]).toUpperCase(); return p[0].substring(0,p[0].length>=2?2:1).toUpperCase();}
  Widget _colorGallery(String name)=> Container(color: _cFor(name), child: Center(child: Padding(padding: const EdgeInsets.all(20), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Container(width: 80, height: 80, decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.22), borderRadius: BorderRadius.circular(18)), child: Center(child: Text(_iniFor(name), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 28)))), const SizedBox(height: 12), Text(name, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 18, height: 1.2))]))));

  Widget _gallerySection(BuildContext context, Product p) {
    // Picture kahit color lang with name (user request)
    return Container(height: 360, decoration: BoxDecoration(color: _cFor(p.name), borderRadius: BorderRadius.circular(16)), clipBehavior: Clip.antiAlias, child: _colorGallery(p.name));
  }
  ...
  Widget _image(String? url) {
    // Now just color with name (per user request)
    final p = _product;
    if (p == null) return const Icon(Icons.image_not_supported_outlined, color: Colors.grey, size: 48);
    return _colorGallery(p.name);
  }
